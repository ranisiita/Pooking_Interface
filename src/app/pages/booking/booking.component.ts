      ...
      next: (res) => {
        ...
        if (res) {
          ...
          const newBookingItem = {
            ...
            clienteNombre: `${this.nombres} ${this.apellidos || ''}`.trim(),
            clienteEmail: this.correo,
            clienteTelefono: this.telefono,
            clienteDireccion: this.direccion
          };
          try {
            const listStr = localStorage.getItem('pooking_lodging_reservations') || '[]';
            const list = JSON.parse(listStr);
            list.push(newBookingItem);
            localStorage.setItem('pooking_lodging_reservations', JSON.stringify(list));
            console.log('[DEBUG] Saved lodging reservation to localStorage:', newBookingItem);
          } catch (e) {
            console.error('Error saving reservation to localStorage:', e);
          }
          ...
          const saveToMiddleware = (resolvedClienteGuid: string) => {
            const middlewarePayload = {
              ...
              guidServicioRef: "48f44e82-2717-412c-90bb-bfe1168c3164",
              nombreServicioSnap: this.provider, // Igual al nombre del servidor/proveedor
              tipoServicioSnap: "alojamiento",   // Siempre alojamiento para hospedaje
            ...
            this.http.post(`${environment.apiGatewayUrl}/api/v2/booking/clientes/reservas`, middlewarePayload).subscribe({
              next: (midRes) => {
                console.log('[DEBUG] Reservation saved to middleware successfully:', midRes);
              },
              error: (midErr) => {
                console.error('[ERROR] Failed to save reservation to middleware:', midErr);
              }
            });
          };
